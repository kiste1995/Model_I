...
importJsOnce("js/viewers/meta/Viewer_RobotMan.js");
...
importJsOnce("js/easeljs.min.js");
importJsOnce("js/eventemitter2.min.js");
importJsOnce("js/mjpegcanvas.min.js");
importJsOnce("js/nav2d.js");
importJsOnce("js/ros2d.min.js");
importJsOnce("js/roslib.min.js");

importJsOnce("js/viewers/RobotCtrlViewer.js");
importJsOnce("js/viewers/RobotStatusViewer.js");

var initOK = false;
var runRobotCtrl = false;
...
let onOpen = function() {
  for(let topic_name in subscriptions) {
    ...
    if(topic_name == "_robot_ctrl")
    {
      initSubscribe2({topicName: "_robot_ctrl", topicType: "RobotCtrl"});
    } else if(topic_name == "_robot_status")
    {
      initSubscribe2({topicName: "_robot_status", topicType: "RobotStatus"});
    }
    else
    {
...
let onTopics = function(topics) {
  ...
  $('<a></a>')
  .addClass("mdl-navigation__link")
  .click(() => { initSubscribe2({topicName: "_robot_ctrl", topicType: "RobotCtrl"}); })
  .text("Robot Control")
  .appendTo($("#robot-man"));

  $('<a></a>')
  .addClass("mdl-navigation__link")
  .click(() => { initSubscribe2({topicName: "_robot_status", topicType: "RobotStatus"}); })
  .text("Robot Status")
  .appendTo($("#robot-man"));
...
function initSubscribe2({topicName, topicType}) {
  // creates a subscriber for topicName
  // and also initializes a viewer (if it doesn't already exist)
  // in advance of arrival of the first data
  // this way the user gets a snappy UI response because the viewer appears immediately
  ...
  if(!subscriptions[topicName].viewer) {
    ...
    try {
      if(topicName == "_robot_ctrl")
      {
        subscriptions[topicName].viewer = new RobotCtrlViewer(card, topicName, topicType);
      }
      else if (topicName == "_robot_status")
      {
        subscriptions[topicName].viewer = new RobotStatusViewer(card, topicName, topicType);
      }
...
Viewer.onClose = function(viewerInstance) {
  ...
  console.log("onClose : " + topicName + " " + topicType);
...
Viewer_RobotMan.onClose = function(viewerInstance) {
  let topicName = viewerInstance.topicName;
  let topicType = viewerInstance.topicType;
  console.log("onClose : " + topicName + " " + topicType);
  currentTransport.unsubscribe({topicName:topicName});
...
Viewer_RobotMan.onSwitchViewer = (viewerInstance, newViewerType) => {
  let topicName = viewerInstance.topicName;
  let topicType = viewerInstance.topicType;
  if(!subscriptions[topicName].viewer === viewerInstance) console.error("viewerInstance does not match subscribed instance");
  let card = subscriptions[topicName].viewer.card;
