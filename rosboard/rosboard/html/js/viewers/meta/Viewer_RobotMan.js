...
//   constructor(card, topicName, topicType) {
    ...
    card.menu = $('<ul class="mdl-menu mdl-menu--bottom-right mdl-js-menu mdl-js-ripple-effect" \
      for="' + menuId + '"></ul>').appendTo(card);
        ...
        let viewers = Viewer_RobotMan.getViewersForType(this.topicType);
    for(let i in viewers) {
      ...
      let that = this;
      item.click(() => { Viewer_RobotMan.onSwitchViewer(that, viewers[i]); });
    }
    ...
    card.closeButton = $('<button></button>')
      ...
      .appendTo(card.buttons);
    card.closeButton.click(() => { Viewer_RobotMan.onClose(that); });
...
Viewer_RobotMan.maxUpdateRate = 50.0;

// not to be overwritten by child class!
// stores registered viewers in sequence of loading
Viewer_RobotMan._viewers = [];

// override this
Viewer_RobotMan.onClose = (viewerInstance) => { console.log("not implemented; override necessary"); }
Viewer_RobotMan.onSwitchViewer = (viewerInstance, newViewerType) => { console.log("not implemented; override necessary"); }

// not to be overwritten by child class!
Viewer_RobotMan.registerViewer = (viewer) => {
  // registers a viewer. the viewer child class calls this at the end of the file to register itself
  Viewer_RobotMan._viewers.push(viewer);
};

// not to be overwritten by child class!
Viewer_RobotMan.getDefaultViewerForType = (type) => {
  ...
  for(let i in Viewer_RobotMan._viewers) {
    if(Viewer_RobotMan._viewers[i].supportedTypes.includes(type)) {
      return Viewer_RobotMan._viewers[i];
    }
    if(Viewer_RobotMan._viewers[i].supportedTypes.includes("*")) {
      return Viewer_RobotMan._viewers[i];
    }
  }
  return null;
...
// not to be overwritten by child class!
Viewer_RobotMan.getViewersForType = (type) => {
  ...
  for(let i in Viewer_RobotMan._viewers) {
    if(Viewer_RobotMan._viewers[i].supportedTypes.includes(type)) {
      matchingViewers.push(Viewer_RobotMan._viewers[i]);
    }
    if(Viewer_RobotMan._viewers[i].supportedTypes.includes("*")) {
      matchingViewers.push(Viewer_RobotMan._viewers[i]);
    }
