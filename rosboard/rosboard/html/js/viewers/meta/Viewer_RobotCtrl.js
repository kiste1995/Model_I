...
Viewer_NaviCtrl.maxUpdateRate = 50.0;

// not to be overwritten by child class!
// stores registered viewers in sequence of loading
Viewer_NaviCtrl._viewers = [];

// override this
Viewer_NaviCtrl.onClose = (viewerInstance) => { console.log("not implemented; override necessary"); }
Viewer_NaviCtrl.onSwitchViewer = (viewerInstance, newViewerType) => { console.log("not implemented; override necessary"); }

// not to be overwritten by child class!
Viewer_NaviCtrl.registerViewer = (viewer) => {
...
// not to be overwritten by child class!
Viewer_NaviCtrl.getDefaultViewerForType = (type) => {
  ...
  for(let i in Viewer_NaviCtrl._viewers) {
    if(Viewer_NaviCtrl._viewers[i].supportedTypes.includes(type)) {
      return Viewer_NaviCtrl._viewers[i];
    }
    if(Viewer_NaviCtrl._viewers[i].supportedTypes.includes("*")) {
      return Viewer_NaviCtrl._viewers[i];
    }
  }
  return null;
...
// not to be overwritten by child class!
Viewer_NaviCtrl.getViewersForType = (type) => {
  ...
  for(let i in Viewer_NaviCtrl._viewers) {
    if(Viewer_NaviCtrl._viewers[i].supportedTypes.includes(type)) {
      matchingViewers.push(Viewer_NaviCtrl._viewers[i]);
    }
    if(Viewer_NaviCtrl._viewers[i].supportedTypes.includes("*")) {
      matchingViewers.push(Viewer_NaviCtrl._viewers[i]);
    }
