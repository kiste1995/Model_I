...
class ROSBoardNode(object):
    ...
    def __init__(self, node_name = "rosboard_node"):
        ...
        # rospy.loginfo("path :%s" % os.path.join(os.path.dirname(os.path.realpath(__file__)), 'html'))
        ...
        try:
            ...
            for topic_name in self.remote_subs:
                ...
                if topic_name == "_robot_ctrl":
                    if topic_name not in self.local_subs:
                        rospy.loginfo("Subscribing to _robot_ctrl [ros]")
                ...
                if topic_name == "_robot_status":
                    if topic_name not in self.local_subs:
                        rospy.loginfo("Subscribing to _robot_status [ros]")
                        self.local_subs[topic_name] = DummySubscriber()
                        self.event_loop.add_callback(
                            ROSBoardSocketHandler.broadcast,
                            [
